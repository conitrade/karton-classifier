...
class TestClassifier(KartonTestCase):
    ...
    def test_process_runnable_win32_dll(self):
        magic, mime = "PE32 executable (DLL) (console) Intel 80386...", "application/x-dosexec"
        self.karton = mock_classifier(magic, mime)
        resource = mock_resource("file")
        res = self.run_task(mock_task(resource))
    ...
    def test_process_runnable_win32_exe(self):
        magic, mime = "PE32 executable (GUI) Intel 80386 Mono/.Net assembly...", "application/x-dosexec"
        self.karton = mock_classifier(magic, mime)
        resource = mock_resource("file")
        res = self.run_task(mock_task(resource))
    ...
    def test_process_runnable_win32_msi(self):
        magic, mime = "Composite Document File V2 Document, MSI Installer...", "application/x-msi"
        self.karton = mock_classifier(magic, mime)
    ...
    def test_process_runnable_win32_swf(self):
        magic, mime = "Macromedia Flash data (compressed)...", "application/x-shockwave-flash"
        self.karton = mock_classifier(magic, mime)
